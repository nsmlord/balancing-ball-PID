...
'''
Author: Nishanth Shubhakar Mandala
Project: PID-Controller Based Ball Balancer

Details:
    This project uses collision-impulses along
    with rotational and classical mechanics as 
    its physics engine. 

    The integrator used for the physics engine 
    is symplectic euler.

    The integrator used for the integral controller 
    is trapezoidal sum.

    A PID-controller is used to modify the platform 
    angle to keep the ball in the middle.

Inputs:
    - Proportional controller gain (proportional)
    - Integral controller gain (integral)
    - Differential controller gain (differential)
    - Slope Angle (slopeAng)
    - Ball position (body['pos'])
Outputs:
    - Real-Time Graphical Simulation
    - Proportional Controller Output vs Time
    - Integral Controller Output vs Time
    - Differential Controller Output vs Time
    - Error vs Proportional Output
    - Slope Angle vs Time

Notes:
    - Energy conservation was not verified
    - Set to display every 150 frames
    - Coefficient of Restitution set to 0 to
      avoid noise in PID feedback; sometimes 
      leads to unrealistic bouncing
'''
...
coefRest = 0  # Coefficient of restitution
R = 0.20  # Radius
mass = 25.0
Icoeff = 2 / 5  # Solid sphere
I = Icoeff * mass * R ** 2
proportional = 0.003  # Proportional gain working: 0.003
differential = 0.3615  # differential gain working: 0.3615
integral = 0.08037  # integral gain working: 0.08037

# Slope specification
slopeAng = 6 * pi / 180  # Initial slope angle
mid_x = 5.0
mid_z = 3.0
reference = np.array([mid_x, mid_z + R])
slopeLen = 10.0
...
for step in range(steps):
    ...
    if step in (0, 1, 2, 3):
        print("bodypos=", body['pos'])
        print("error=", error_along_slope)
        print("error_deriv=", error_dot)
        print("error_integ=", error_integral)
...
fig = plt.figure(figsize=(15, 10), facecolor=(0.6, 0.6, 0.6))
...
ax_sim.set_title('Balancing Ball Simulation with PID Controller')
...
# Bottom left: Error vs Proportional output
ax_err_prop = fig.add_subplot(gs[2, 0])
err_prop_line, = ax_err_prop.plot([], [], 'purple')
ax_err_prop.set_xlabel('Proportional Output (P)')
ax_err_prop.set_ylabel('Error')
ax_err_prop.set_title('Error vs Proportional Output')
ax_err_prop.grid(True)
if len(p_output) > 0:
    p_min, p_max = min(p_output), max(p_output)
    if p_min == p_max:
        p_min -= 1e-6
        p_max += 1e-6
    ax_err_prop.set_xlim(p_min * 1.1, p_max * 1.1)
if len(error) > 0:
    e_min, e_max = min(error), max(error)
    if e_min == e_max:
        e_min -= 1e-6
        e_max += 1e-6
    ax_err_prop.set_ylim(e_min * 1.1, e_max * 1.1)
...
def init():
    ...
    i_line.set_data([], [])
    d_line.set_data([], [])
    err_prop_line.set_data([], [])
    slope_line_plot.set_data([], [])
    return circle, line, slope_line, p_line, i_line, d_line, err_prop_line, slope_line_plot
...
def animate(i):
    ...
    # Update slope
    tHat_local, nSlope_local, dSlope_local, P1_local, P2_local = update_slope_geometry(slope_angles[i])
    slope_line.set_data([P1_local[0], P2_local[0]], [P1_local[1], P2_local[1]])

    # Current data up to animation frame
    current_step = i * display_every
    current_time = current_step * dt  # kept for clarity, not used below
